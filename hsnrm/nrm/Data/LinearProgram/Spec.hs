...
import Protolude (NFData, deepseq, rnf)
...
allVars LP {..} =
  foldl
    union
    ((() <$ objective) `union` (() <$ varBounds) `union` (() <$ varTypes))
...
showFunc func = case assocs func of
  ...
  where
    showTerm (v, c) = case compare c 0 of
      EQ -> ""
      GT -> " + " ++ show c ++ " " ++ show v
...
instance (Read v, Ord v, Read c, Ord c, Num c, Group c) => Read (Constraint v c) where
  readsPrec _ = readP_to_S $ fmap toConstr (lab <++ nolab)
    where
      ...
      nolab = (\(f, bds) -> (Nothing, f, bds)) <$> readBds readConst readFunc
      readFunc =
        ( do
            c <- readCoef readConst
            v <- readVar
            fmap ((v, c) :) readFunc
        )
          <++ return []
...
instance (NFData v, NFData c) => NFData (LP v c) where
  rnf LP {..} =
    direction `deepseq` objective `deepseq` constraints
      `deepseq` varBounds
      `deepseq` rnf varTypes
