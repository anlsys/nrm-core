...
module Data.LinearProgram.GLPK.IO.Internal
  ( readGLPLP,
    writeGLPLP,
  )
where
...
getDir = toEnum . subtract 1 . fromIntegral <$> GLP glpGetObjDir
...
peekCAString' str
  ...
  | otherwise = Just <$> peekCAString str

getNumRows, getNumCols :: GLPK Int
getNumRows = fromIntegral <$> GLP glpGetNumRows
getNumCols = fromIntegral <$> GLP glpGetNumCols
...
colKind = fmap (toEnum . subtract 1) . getCInt glpGetColKind

getCInt :: (Ptr GlpProb -> CInt -> IO CInt) -> Int -> GLPK Int
getCInt f i = GLP $ \lp -> fromIntegral <$> f lp (fromIntegral i)

getCDouble :: (Ptr GlpProb -> CInt -> IO CDouble) -> Int -> GLPK Double
getCDouble f i = GLP $ \lp -> realToFrac <$> f lp (fromIntegral i)
...
getRows = do
  ...
  sequence
    [ do
        k <- fmap fromIntegral $ GLP $ \lp -> glpGetMatRow lp (fromIntegral i) ixs coefs
  ...
  sequence_
    [ do
        ...
        maybe
          ...
          constrain'
          name
          (linCombination [(v, names ! j) | (j, v) <- row])
          bds
      | (i, row) <- rowContents
    ]
  obj <-
