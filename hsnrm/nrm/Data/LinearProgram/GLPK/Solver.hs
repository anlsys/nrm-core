...
-- problem, a value of type 'LP', and the GLPK solver.  The options available
-- to the solver correspond naturally with GLPK's available options, so to find
-- the meaning of any particular option, consult the GLPK documentation.
--
-- The option of which solver to use -- the general LP solver, which solves a
-- problem over the reals, or the MIP solver, which allows variables to be
-- restricted to integers -- can be made by choosing the appropriate
-- constructor for 'GLPOpts'.
--
-- The marshalling from Haskell to C is specialized for 'Int's and 'Double's,
-- so using those types in your linear program is recommended.
...
data GLPOpts
  = SimplexOpts {msgLev :: MsgLev, tmLim :: !Int, presolve :: Bool}
  | MipOpts
      { msgLev :: MsgLev,
        tmLim :: !Int,
        presolve :: Bool,
        brTech :: BranchingTechnique,
        btTech :: BacktrackTechnique,
...
{-# SPECIALIZE glpSolveVars ::
  Ord v => GLPOpts -> LP v Double -> IO (ReturnCode, Maybe (Double, Map v Double)),
  ...
  (code,)
    <$> maybe
      ...
      ( \vars -> do
          ...
          return (Just (obj, fromDistinctAscList vals))
      )
      vars
  ...
  (code,)
    <$> maybe
      ...
      ( \vars -> do
          ...
          rows <-
            sequence
              [ RowVal c <$> getRowPrim i
                | (i, c) <- zip [1 ..] constraints
              ]
          return (Just (obj, fromDistinctAscList vals, rows))
      )
      vars
...
{-# SPECIALIZE doGLP ::
  Ord v => GLPOpts -> LP v Double -> GLPK (ReturnCode, Maybe (Map v Int)),
