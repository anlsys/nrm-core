...
{-# SPECIALIZE equal ::
  (Ord v, Group c) => LinFunc v c -> LinFunc v c -> LPM v c (),
...
{-# SPECIALIZE equal' ::
  (Ord v, Group c) => String -> LinFunc v c -> LinFunc v c -> LPM v c (),
...
constrain f bds = modify addConstr
  where
    addConstr lp@LP {..} =
      lp {constraints = Constr Nothing f bds : constraints}

{-# SPECIALIZE constrain' ::
  String -> LinFunc v c -> Bounds c -> LPM v c (),
...
constrain' lab f bds = modify addConstr
  where
    addConstr lp@LP {..} =
      lp {constraints = Constr (Just lab) f bds : constraints}

{-# SPECIALIZE setObjective ::
  LinFunc v c -> LPM v c (),
...
setObjective obj = modify setObj
  where
    setObj lp = lp {objective = obj}

{-# SPECIALIZE addObjective ::
  (Ord v, Group c) => LinFunc v c -> LPM v c (),
...
addObjective obj = modify addObj
  where
...
-- @'addObjective' (wt '*^' obj)@.
addWeightedObjective ::
  (Ord v, Ring c, MonadState (LP v c) m) =>
  c ->
  LinFunc v c ->
  m ()
addWeightedObjective wt obj = addObjective (wt *^ obj)

{-# SPECIALIZE setVarBounds ::
  (Ord v, Ord c) => v -> Bounds c -> LPM v c (),
...
setVarBounds var bds = modify addBds
  where
    addBds lp@LP {..} = lp {varBounds = insertWith mappend var bds varBounds}

{-# SPECIALIZE setVarKind ::
  Ord v => v -> VarKind -> LPM v c (),
