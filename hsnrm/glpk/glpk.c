...
void c_glp_maximize(glp_prob *lp) { glp_set_obj_dir(lp, GLP_MAX); }

void c_glp_minimize(glp_prob *lp) { glp_set_obj_dir(lp, GLP_MIN); }

// void c_glp_set_obj_dir(glp_prob *lp, int dir){
//   	glp_set_obj_dir(lp, dir);
// }

int c_glp_get_bad_ray(glp_prob *lp) { return glp_get_unbnd_ray(lp); }

int c_glp_add_rows(glp_prob *lp, int nrows) { return glp_add_rows(lp, nrows); }

int c_glp_add_cols(glp_prob *lp, int ncols) { return glp_add_cols(lp, ncols); }
...
void c_glp_set_mat_row(glp_prob *lp, int i, int len, const int ind[],
                       const double val[]) {
  glp_set_mat_row(lp, i, len, ind, val);
}

void c_glp_delete_prob(glp_prob *lp) { glp_delete_prob(lp); }

void c_glp_create_index(glp_prob *lp) { glp_create_index(lp); }
...
int c_glp_mip_solve(glp_prob *lp, int msg_lev, int br_tech, int bt_tech,
                    int pp_tech, int fp_heur, int tm_lim, int cuts,
                    double mip_gap, int presolve) {
  glp_iocp iocp;
  glp_mem_limit(10000);
...
int c_glp_get_obj_dir(glp_prob *lp) { return glp_get_obj_dir(lp); }

int c_glp_get_num_rows(glp_prob *lp) { return glp_get_num_rows(lp); }
...
double c_glp_get_row_lb(glp_prob *lp, int i) { return glp_get_row_lb(lp, i); }

double c_glp_get_row_ub(glp_prob *lp, int i) { return glp_get_row_ub(lp, i); }

double c_glp_get_col_lb(glp_prob *lp, int i) { return glp_get_col_lb(lp, i); }

double c_glp_get_col_ub(glp_prob *lp, int i) { return glp_get_col_ub(lp, i); }

int c_glp_get_row_type(glp_prob *lp, int i) { return glp_get_row_type(lp, i); }
